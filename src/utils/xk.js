...
export function doMove(obj, attr, dir, target, endFn) {
...
export function shake(obj, attr, endFn) {
...
export function hide(obj, cy, sec, endFn) {
    let timer = null;
...
export function out(obj, cy, sec, endFn) {
    let timer = null;
    let fadeNum = Number(getStyle(obj, 'opacity') * 100);
    let fadeNum1 = Number(getStyle(obj, 'opacity'));
    timer = setInterval(function () {
...
*/
export function getPos(obj) { //判断的就是盒子，如果循环到最上面时，盒子就无法判断，也就为false
...
export function drag(obj) {
    obj.onmousedown = function (event) { //1、先按下
        let ev = event || window.event; //兼容ev
        ...
        document.onmousemove = function (event) { //2、按下时移动
...
/*
	...
	export function end()								//这里是任务，会开始继续加宽100px
	{
	...
	//兼容处理
	export function addEnd(obj,fn)		//过渡完成后加事件
	{
		obj.addEventListener('WebkitTransitionEnd',fn,false);
		obj.addEventListener('transitionend',fn,false);
	}

	export function removeEnd(obj,fn)	//删除事件
	{
